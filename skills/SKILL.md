...
version: 2.3.2
metadata: { "openclaw": { "emoji": "📖", "requires": { "bins": ["inkos", "node"], "env": ["OPENAI_API_KEY"] }, "primaryEnv": "OPENAI_API_KEY", "homepage": "https://github.com/Narcooo/inkos", "install": [{ "id": "npm", "kind": "node", "package": "@actalk/monarch", "label": "Install InkOS (npm)" }] } }
---
...
- **License**: the ClawHub skill descriptor is MIT-0 per platform policy, but the underlying `@actalk/monarch`, `@actalk/monarch-core`, and `@actalk/monarch-studio` npm packages are **AGPL-3.0-only**. Running InkOS and distributing modified versions are governed by AGPL. Full source on GitHub for auditability.
