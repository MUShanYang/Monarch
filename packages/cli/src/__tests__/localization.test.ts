...
describe("CLI localization", () => {
  it("formats book-create summaries in both languages", () => {
    ...
    expect(formatBookCreateNextStep("zh", "shan-he")).toBe("下一步：monarch write next shan-he");
    ...
    expect(formatBookCreateNextStep("en", "harbor")).toBe("Next: monarch write next harbor");
  });
  ...
  it("formats import summaries with language-specific units and action hints", () => {
    ...
    expect(formatImportChaptersComplete("zh", {
      ...
      "  下一章编号：13",
      "",
      '运行 "monarch write next shan-he" 继续写作。',
    ]);
    ...
    expect(formatImportChaptersComplete("en", {
      ...
      "",
      'Run "monarch write next harbor" to continue writing.',
    ]);
  });
