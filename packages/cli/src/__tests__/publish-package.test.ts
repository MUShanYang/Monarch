...
describe.sequential("publish packaging", () => {
  it("rewrites workspace package versions for canary publishing", async () => {
    ...
    try {
      ...
      await writeFile(
        ...
        `${JSON.stringify({ name: "@actalk/monarch-core", version: "0.4.6" }, null, 2)}\n`,
      );
      await writeFile(
      ...
      expect(cliPackageJson.dependencies["@actalk/monarch-core"]).toBe("0.4.8-canary.7");
    } finally {
  ...
  it("keeps source CLI dependencies linked through the workspace protocol", async () => {
    ...
    expect(cliPackageJson.dependencies["@actalk/monarch-core"]).toBe("workspace:*");
    expect(cliPackageJson.dependencies["@actalk/monarch-studio"]).toBe("workspace:*");
  });
  ...
  it("allows source workspace protocol manifests when they normalize cleanly for publish", async () => {
    ...
    try {
      ...
      await writeFile(
        join(tempCoreDir, "package.json"),
        `${JSON.stringify({ name: "@actalk/monarch-core", version: "0.5.1" }, null, 2)}\n`,
      );
      await writeFile(
        join(tempCliDir, "package.json"),
        `${JSON.stringify(
        ...
        `${JSON.stringify(
          {
            ...
            version: "0.5.1",
            dependencies: {
              "@actalk/monarch-core": "workspace:0.5.0",
            },
          },
          null,
          2,
  ...
  it("replaces workspace dependencies before npm pack", { timeout: 30_000 }, async () => {
    ...
    try {
      ...
      expect(packedPackageJson.dependencies["@actalk/monarch-core"]).toBe(corePackageJson.version);
      expect(packedPackageJson.dependencies["@actalk/monarch-studio"]).toBe(studioPackageJson.version);
    } finally {
