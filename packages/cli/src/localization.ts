import { formatLengthCount, resolveLengthCountingMode } from "@actalk/monarch-core";
...
export function formatBookCreateNextStep(language: CliLanguage, bookId: string): string {
  return localize(language, {
    zh: `下一步：monarch write next ${bookId}`,
    en: `Next: monarch write next ${bookId}`,
  });
}
...
export function formatImportChaptersComplete(
  ...
  return [
    ...
    "",
    localize(language, {
      zh: `运行 "monarch write next ${result.continueBookId}" 继续写作。`,
      en: `Run "monarch write next ${result.continueBookId}" to continue writing.`,
    }),
  ];
}
