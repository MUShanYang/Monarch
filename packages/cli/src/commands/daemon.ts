...
import { Scheduler } from "@actalk/monarch-core";
import { loadConfig, findProjectRoot, buildPipelineConfig, log, logError } from "../utils.js";
import { createWriteStream, type WriteStream } from "node:fs";
import { writeFile, readFile, unlink } from "node:fs/promises";
import { join } from "node:path";

const PID_FILE = "monarch.pid";

export const upCommand = new Command("up")
  .description("Start the Monarch daemon (autonomous mode)")
  ...
  .action(async (opts) => {
    ...
    try {
      ...
      try {
        ...
        logError(`Daemon already running (PID: ${existingPid.trim()}). Run 'monarch down' first.`);
        process.exit(1);
      } catch {
        // No PID file, good
      }

      log("Starting Monarch daemon...");
      ...
      const logPath = join(root, "monarch.log");
...
export const downCommand = new Command("down")
  .description("Stop the Monarch daemon")
  .action(async () => {
    const root = findProjectRoot();
