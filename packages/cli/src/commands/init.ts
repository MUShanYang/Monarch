...
export const initCommand = new Command("init")
  .description("Initialize a Monarch project (current directory by default)")
  ...
  .action(async (name: string | undefined, opts: { lang?: string }) => {
    ...
    try {
      ...
      const exampleCreate = isEnglish
        ? "  monarch book create --title 'My Novel' --genre progression --platform royalroad --lang en"
        : "  monarch book create --title '我的小说' --genre xuanhuan --platform tomato";
      if (global) {
        ...
        log("  monarch config set-global --provider openai --base-url <your-api-url> --api-key <your-key> --model <your-model>");
        log("  # Option 2: Edit .env for this project only");
        log("");
        log("Next steps:");
      }
      log("  monarch write next <book-id>");
    } catch (e) {
